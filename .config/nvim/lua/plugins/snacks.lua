...
return {
  ...
  opts = {
    ...
    dashboard = {
      ...
      preset = {
        keys = {
          ...
          { icon = ' ', key = 'c', desc = 'Conf', action = ":lua Snacks.dashboard.pick('files', {cwd = vim.fn.stdpath('config')})" },
    ...
    indent = {
      ...
      filter = function(buf)
        local exclude = { "java", "typescript", "markdown", "go", "lazy", "snacks_dashboard", "dbout" }
